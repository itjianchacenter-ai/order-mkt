...
const menuPublic = { banner: menu.banner || '', stock: menu.stock || {}, promo_code: menu.promo_code || {}, pickup_dates: Array.isArray(menu.pickup_dates) ? menu.pickup_dates : [], pickup_times: Array.isArray(menu.pickup_times) ? menu.pickup_times : [], order_days: Array.isArray(menu.order_days) ? menu.order_days : [], orders_per_day: menu.orders_per_day || null, pickup_map: menu.pickup_map && typeof menu.pickup_map === "object" ? menu.pickup_map : {}, sets: (menu.sets || []).filter((x) => x.active !== false).map(normSet), drinks: (menu.drinks || []).filter((x) => x.active !== false), desserts: (menu.desserts || []).filter((x) => x.active !== false) };
